...
class UserRoutes {
    ...
    initRoutes() {
         ...
         * - role: string (one of "resident", "urban_developer", "urban_planner").
         * It returns a 200 status code.
        this.router.post(
            "/",
            body("username").isString().isLength({ min: 1 }),
            body("password").isString().isLength({ min: 1 }),
            body("role").isString().isIn(["resident", "urban_developer", "urban_planner"]),
        ...
        this.router.get(
            ...
            .then((users: Users[]) => res.status(200).json(users))
            .catch((err) => next(err))
        )
        */
        ...
        this.router.delete(
            ...
            (req: any, res: any, next: any) => {
                ...
                .then((userToDelete: User) => {
                    if (req.user.role === "Admin" && userToDelete.role === "Admin" && req.user.username!== userToDelete.username){
        ...
        this.router.patch(
            ...
            (req: any, res: any, next: any) => {
                ...
                .then((userToUpdate: User) => {
                    if (req.user.role === "Admin" && userToUpdate.role === "Admin" && req.user.username!== userToUpdate.username){
                        throw new UserIsAdminError
                    }
    ...
    initRoutes() {
        ...
        this.router.post(
            ...
            async (req: any, res: any, next: any) => {
                try {
                    const user: User = await this.authService.login(req, res, next);
